...
function checkLocalStorage(window){
  try {
    ...
    cache = JSON.parse(window.localStorage.getItem('blackbox')) || {};
    return true;
  } catch(e) {
    return false;
  }
...
Batch.prototype.timeout = function() {
  var self = this;
  !this.timer && (this.timer = setTimeout(function() { self.send(); self.timer = null; }, this.interval));
};
...
Batch.prototype.write = function(item) {
  ...
  hasLocalStorage && window.localStorage.setItem('blackbox', SafeJSON.stringify(cache));
};
...
Batch.prototype.send = function() {
  var messages = [], key, self = this;
  if(!enabled) return;
  for(key in cache) {
    messages.concat(cache[key]);
  }

  if(messages.length < 1) return timeout();
